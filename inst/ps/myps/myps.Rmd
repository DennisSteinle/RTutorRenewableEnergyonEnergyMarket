...
Nowadays a central and interesting question for discussion is global warming and the problem with limited resources of fossil fuels. Looking at this part of political topics, the first solar cells have been installed in the USA in the 1950s. The project of energy generation by the wind already started at the end of the 19th century. But at the beginning of the 21st century, a big focus was directed to renewable energy and how you could use them to create a new sector with the option to produce electricity without limited fuels. 
...
Quiz: What do you think, how many different variables does our whole dataset have?
...
Therefore, we often use the `head()` function. In our situation, we are interested in the first 10 observations. To run this call, you first have to name the variable name, where you saved your dataset. As the second argument you must insert the number of rows you want to view, in our case it is 10. 
...
Quiz: What is the time range of our dataset?
...
Quiz: How many observations has our dataset?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Duration of the experiment")
```
...
Quiz: How many observations has our dataset now?
...
Quiz: Does electricity production by solar increase steadily over the years?

[1]: Yes.
[2]: No.
[3]: No, but wind generation.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Development")
```
...
Quiz: What do you think? Has solar power the highest increase of all sources?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Power of solar generation")
```
...
```{r "4_6",fig.width=10, fig.height=7}
...
Quiz: What can you observe?

[1]: Renewable energies are falling and conventional power generation remains constant.
[2]: Renewable energies are generally increasing and conventional power generation is decreasing.
[3]: Renewable energies and conventional power generation are increasing.
...
Quiz: From 2013 to 2016 the amount of renewable energy is increasing.
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Development of renewable energy")
```
...
Quiz: On which level does the renewable energy production by solar grow?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Extent of increase")
```
...
```{r "4_8",fig.width=10, fig.height=7}
plot <- data_year %>%
...
Quiz: What do you think, at what time is the biggest difference in electricity production by solar?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Daily solar production")
```
...
Quiz: At what time is the biggest difference in electricity production by the wind?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Daily wind production")
```
...
Quiz: The total price of electricity…?

[1]: increases.
[2]: remains the same level.
[3]: falls significantly.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Development of price")
```
...
Quiz: What is the main reason for the development of price?
...
Quiz: Watching the first regression, an increase of solar by 1 GWh corresponds to a decrease in price by…?
...
Quiz: If we control for load the estimator for solar will...?
...
Here, we can see that the estimator for solar is smaller than in the first regression because we have controlled for the impact of load on price and solar.
...
Quiz: Watching the third regression, does the wind have a significantly negative effect on price?

[1]: Yes.
[2]: No.
[3]: Can´t be read in the table.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Effects of regressions")
```


Quiz: How can you see that in regression 3 wind has a significant impact on price?

[1]: Behind the estimator are 3 stars.
[2]: Because the estimator for wind is greater than for solar.
[3]: Not at all.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Significant effects")
```
...
Quiz: The impact of DAOH solar and wind generation is smaller than the impact of the hourly generation by wind and solar?
...
Quiz: Which problems can you observe?

[1]: In the evening and night hours we have inaccurate estimators.
[2]: At noon we have no confidence intervals.
[3]: I can´t see anything.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Problems of hourly values")
```
...
Quiz: An increase of hourly solar generation by 1 GWh at noon reduces the price by?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz(" Impact of hourly solar on price")
```
...
Quiz: Why do we have such a price development? Which of the following reason do the authors mention?

[1]: The electricity composition from gas production changes significantly.
[2]: The estimation results are not significant.
[3]: Solar power is produced, which we cannot observe.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Late evening hours")
```
...
Quiz: An increase of hourly wind generation by 1 GWh at noon reduces the price by?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz(" Impact of wind on price")
```
...
Quiz: At which time of the day do gas power plants run more frequently?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz(" Impact of gas price on wholesale price")
```



Quiz: An increase of gas price by 1 $/MMBtu at 6 am increases the price of energy by...?
...
Quiz: Which form of energy generation has the largest decrease if we generate an additional GWh of hourly solar?
...
```{r "8_1",fig.width=10, fig.height=5}
library(dplyr)
library(ggplot2)
...
Quiz: The impact of DAOH solar generation on power plants is higher than the hourly effects?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Solar effect on power plants")
```


Quiz: What do the authors mention, that is the main reason for the strong decrease in CCGT generation?

[1]: Low fixed costs.
[2]: They are inefficient if they run at low levels.
[3]: They are quick-start plants.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Decrease of CCGT generation")
```
...
Quiz: Which source of the power plants is strongly impacted by a higher wind production regardless of the time of day?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Gas generation")
```



Quiz: Which of the following reason do the authors mention is most important?

[1]: CCGT is a baseload power plant.
[2]: Constant wind generation.
[3]: High demand in the evening.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Reason for this impact")
```
...
Quiz: In fact, we can observe that .... an additional GWh of wind energy leads to the strongest decrease of GT and ST generation?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Decrease of fossil generation")
```
...
Quiz: An increase of 1 GWh by DAOH solar leads to the strongest decline in ...?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Power plants")
```


Quiz: An increase of 1 GWh by DAOH solar output must not offset 1 GWh of daily output from all other sources?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Daily power generation")
```
...
Quiz: When we have no solar power between 8 pm and 6 am, then the net change in electricity supplied by all other sources must be equal to zero?

[1]: Right.
[2]: Wrong.
[3]: Not in this time range.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Power plants during night hours")
```
...
Now, we want to focus on the hours, where solar power is generated. We compare the hourly and daily average effect of an additional GWh of solar power on our four energy sources between 8 am and 6 pm.

Task: Just press check.


```{r "9_2",fig.width=10, fig.height=7}
y_plot <- results_reg %>%
...
Quiz: Which 2 sources have a similar course of the DAOH and hourly curves?

[1]: Thermal and Nuclear.
[2]: Hydro and Imports.
[3]: Nuclear and Imports.
[4]: Hydro and Thermal.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Output by sources")
```

We already answered the effect between the declines in electricity production by thermal, hydro, nuclear, imports, and additional solar production in the first task. Now, we want to look at how the curves run in general, where the effect is greater and what could be the reason, therefore.
...
Quiz: Which source of the power plants is strongly affected by a higher wind production?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Wind on conventional")
```


Quiz: At what time does higher wind production have the greatest effect on all sources?

[1]: in the morning hours.
[2]: in the early afternoon hours.
[3]: at night.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Wind at different times of day")
```
...
```{r "9_3",fig.width=10, fig.height=7}
y_plot <- results_reg %>%
...
Quiz: Which 2 seasons have the highest average solar generation in 2016?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Seasonal solar generation")
```
...
```{r "10_1",fig.width=10, fig.height=8}
library(ggplot2)
library(dplyr)
...
Quiz: Which 2 seasons have the highest average wind generation in 2013?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Seasonal wind generation")
```
...
```{r "10_2",fig.width=8, fig.height=7}
...
Quiz: In which year and season do we reach the maximum electricity price?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Seasonal energy price")
```
...
```{r "10_3",fig.width=8, fig.height=7}
data_plot <- data_new %>%
...
Quiz: Do you think we can observe our "evening effect" in every season?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Solar in seasons")
```



Quiz: In which season leads an additional GWh of electricity to the sharpest price decline?
...
```{r "10_4",fig.width=8, fig.height=7}
...
Quiz: What is a possible reason for the price decrease in the winter by an additional GWh of DAOH solar generation?

[1]: High demand.
[2]: Energy generation with high marginal costs and low solar generation.
[3]: Low supply.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Reason for price decrease")
```
...
Quiz: What do you think, do we have a positive price effect in the evening hours?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Wind impact on the price")
```
...
Quiz: In which season leads an additional GWh wind to the largest price decrease?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Effects of wind on price")
```
...
```{r "10_5",fig.width=8, fig.height=7}
y_plot <- data %>%
...
Quiz: Why does an additional GWh of wind power lead to a sharp price drop at noon, but a slight price increase in the evening and the season´s summer and spring?

[1]: High wind production at noon and low demand in the evening.
[2]: High wind production at noon and high demand in the evening.
[3]: Low wind production at noon and high demand in the evening.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Seasonal effects of the wind generation")
```
...
Quiz: What do you think, are our estimates robust against curtailments?
...
Quiz: Where does the line of the original OLS estimate differ more from the point estimators of the regression with the potential values?

[1]: In solar regressions.
[2]: In wind regressions.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Difference between daily average of hourly and potential values")
```



Quiz: The differences are always in the 95% confidence interval?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Differences of estimators in CI")
```
